...
class RoomController extends Controller
{
    public function index(Request $request)
    {
        ...
        $data['renter'] = RoomForRents::join('renters', 'room_for_rents.ref_renter_id', '=', 'renters.id')
                                        ->whereNotIn('room_for_rents.ref_room_id', $contract_room_has)
                                        ->select('renters.*')
                                        ->distinct()
                                        ->orderBy('renters.id')
                                        ->get();
    ...
    public function insert_contract(Request $request)
    {
        ...
        try{
            ...
            // return $row;
                ...
                $r_b = new RentBill;
                $r_b->ref_room_for_rent_id  =  $row['ref_room_for_rent_id'];
                $r_b->month  =  date('m')-1;
                $r_b->year  =  date('Y');
