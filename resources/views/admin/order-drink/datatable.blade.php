        ...
        @foreach ($orderDrinks as $order)
            <tr>
                <td class="text-center">
                    {{ $loop->iteration + ($orderDrinks->currentPage() - 1) * $orderDrinks->perPage() }}</td>
                    ...
                    <div class="dropdown">
                        ...
                        <ul class="dropdown-menu" aria-labelledby="actionDropdown{{ $order->id }}">
                            ...
                            <li><a class="dropdown-item text-danger" href="#"
                                    onclick="cancelOrder({{ $order->id }}); return false;">ยกเลิกคำสั่งซื้อ</a></li>
                        </ul>
                    </div>
                </td>
            </tr>
...
<script>
    function confirmOrder(orderId) {
        ...
        `,
            ...
            if (result.isConfirmed) {
                ...
                fetch(`/admin/order-drinks/${orderId}/confirm-payment`, {
                        method: 'POST',
                        headers: {
    ...
    function cancelOrder(orderId) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                fetch(`/admin/order-drinks/${orderId}/status`, {
                        method: 'POST',
                        headers: {
                            'X-CSRF-TOKEN': '{{ csrf_token() }}',
                            'Content-Type': 'application/json'
...
{{-- Pagination --}}
<div class="mt-3">
    {!! $orderDrinks->links('vendor.pagination.custom') !!}
</div>
