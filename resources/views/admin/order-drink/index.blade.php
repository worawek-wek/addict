...
<head>
    ...
    <title>การขายดื่ม (Order Drinks)</title>
</head>
<style>
        @media print {
            body { margin: 0; }
...
<body>
                                        ...
                                        <div class="row g-3 justify-content-between">
                                            <div class="col-sm-12">
                                                <h4 class="mb-0">
                                                    ...
                                                    การขายสินค้า (Order Drinks)
                                                </h4>
                                            </div>
                                            <div class="row g-3">
                                                <div class="col-sm-3 mb-2">
                                                    <select name="branch_id" class="form-select p_search"
                                                        onchange='loadData("{{ route('order-drinks.datatable') }}")'>
                                                ...
                                                <div class="col-sm-3 mb-2">
                                                    <div class="input-group input-group-merge">
                                                        ...
                                                        <input
                                                            oninput='loadData("{{ route('order-drinks.datatable') }}")'
    ...
    <script>
        var page = "{{ route('order-drinks.datatable') }}";
        var searchData = {};
        loadData(page);
        ...
        function closures() {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    fetch(`/admin/order-drinks/closures`, {
                        method: 'POST',
                        headers: {
        ...
        function view(id) {
            $.ajax({
                type: "GET",
                url: "{{ route('order-drinks.index') }}/" + id,
                success: function(data) {
                    $("#view").html(data);
