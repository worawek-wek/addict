    ...
    <div class="authentication-wrapper authentication-cover authentication-bg">
      <div class="authentication-inner row">
        ...
        <div class="d-flex col-12 col-lg-8 align-items-center px-sm-5">
          <form id="apartmentAdd" >
            {{-- action="{{ url('apartment/manage') }}" --}}
            @csrf
            <div class="row g-3">
                ...
                <div class="col-sm-12">
                    <label>ชื่อหอพัก <span class="text-danger">*</span></label>
                    <input name="name" type="text" class="form-control"
                        placeholder="กรอกชื่อหอพัก" required/>
                </div>
                <div class="col-sm-12">
                    <label>ที่อยู่หอพัก <span class="text-danger">*</span></label>
                ...
                <div class="col-sm-12">
                    <label>ระบุตำแหน่งหอพักของคุณ</label>
                    <div class="leaflet-map" id="dragMap"></div>
                </div>
                <div class="col-sm-6">
                    <label>เบอร์โทรติดต่อหอพัก <span class="text-danger">*</span></label>
                    <input name="phone" type="text" class="form-control" id="" placeholder="เบอร์โทรติดต่อหอพัก" required/>
                </div>
                <div class="col-sm-6">
                    <label>อีเมลติดต่อหอพัก </label>
                    <input name="email" type="email" class="form-control" id="" placeholder="อีเมลติดต่อหอพัก" />
                </div>
                <div class="col-sm-6">
                    <label>หอพักของคุณทำบิลทุกวันที่เท่าไหร่
                        ?</label>
                ...
                <div class="col-sm-6">
                    ...
                    <button type="submit" class="btn btn-main mt-5" style="margin-bottom: 10%;"><i class="ti ti-plus"></i> สร้างหอพัก</button>
            </div>
        </form>
        </div>
        <!-- /Login -->
...
<script>
    $('#apartmentAdd').on('submit', function(event) {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: '{{"apartment/add"}}', // เปลี่ยน URL เป็นจุดหมายที่ต้องการ
                    type: 'POST',
                    data: $(this).serialize(),
                    success: function(response) {
                        if(response == true){
                            $('#apartmentAdd')[0].reset();
                            Swal.fire('เพิ่มสาขาเรียบร้อยแล้ว', '', 'success');
                            setTimeout(function() {
                                window.location.href = '{{url("/apartment/manage")}}';
                            }, 5000);
                        }
                    },
