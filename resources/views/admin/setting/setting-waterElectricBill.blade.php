                                ...
                                <div class="card mb-3">
                                    <div class="card-header">
                                        <div class="row g-3 justify-content-between">
                                            <div class="col-sm-12">
                                                <h4 class="mb-0">
                                                    <i class="tf-icons ti ti-plug text-main ti-md"></i>
                                                    ค่าน้ำ / ค่าไฟ
                                                </h4>
                                            </div>
                                        </div>
...
<form id="update_water">
    @csrf
    <div class="modal fade modalHeadDecor" id="setWaterModal" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered modal-lg  modal-dialog-scrollable" role="document">
            <div class="modal-content rounded-0">
                <div class="modal-header rounded-0">
                    <h5 class="modal-title" id="exampleModalLabel1">กำหนดค่าน้ำ</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body" id="view_water">
...
<form id="update_electric">
    @csrf
    <div class="modal fade modalHeadDecor" id="setElectricModal" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered modal-lg  modal-dialog-scrollable" role="document">
            <div class="modal-content rounded-0">
                <div class="modal-header rounded-0">
                    <h5 class="modal-title" id="exampleModalLabel1">กำหนดค่าไฟ</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body" id="view_electric">
    ...
    <script>
        ...
        $('#update_water').on('submit', function(event) {
...
// console.log(update_id);
            ...
            Swal.fire({
                ...
                text: 'คุณต้องการแก้ไขค่าน้ำหรือไม่?',
                icon: 'warning',
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: 'setting/water-bill/update/'+update_id, // เปลี่ยน URL เป็นจุดหมายที่ต้องการ
                        type: 'POST',
                        data: $(this).serialize(),
                        success: function(response) {
                            if(response == true){
                                Swal.fire('แก้ไขค่าน้ำเรียบร้อยแล้ว', '', 'success');
                                $('#setWaterModal').modal('hide');
                                loadData(page);
                            }
                        },
        ...
        $('#update_electric').on('submit', function(event) {
            event.preventDefault(); // ป้องกันการส่งฟอร์มปกติ
            if(!this.checkValidity()) {
                // ถ้าฟอร์มไม่ถูกต้อง
                this.reportValidity();
...
// console.log(update_id);
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: 'setting/electric-bill/update/'+update_id, // เปลี่ยน URL เป็นจุดหมายที่ต้องการ
                        type: 'POST',
                        data: $(this).serialize(),
                        success: function(response) {
                            if(response == true){
